...
export function generateStoryDeepLink(
  ...
  return `https://app.pacifica.fi/api/story?${params.toString()}`;
}
...
export const shareTexts = {
  twitter: (data: StoryData) =>
    `${countryFlag(data.countryCode)} ${data.countryName} Global Intel Brief\n\n` +
    `Instability Index: ${data.cii?.score || 'N/A'}/100 (${data.cii?.level || 'N/A'})\n` +
    `${data.threats.critical > 0 ? `⚠️ ${data.threats.critical} critical signals\n` : ''}` +
    `\nPowered by Pacifica • @pacifica_fi`,
  ...
  whatsapp: (data: StoryData) =>
    ...
    `\n📊 View full analysis on Pacifica: ${generateStoryDeepLink(data.countryCode, 'ciianalysis', data.cii?.score, data.cii?.level)}`,

  linkedin: (data: StoryData) =>
    `Global Intel Update: ${data.countryName}\n\n` +
    `Real-time instability assessment from Pacifica:\n` +
    ...
    `\nData via Pacifica Global Intel - Open source geopolitical intelligence`,

  telegram: (data: StoryData) =>
    `${countryFlag(data.countryCode)} *${data.countryName} Global Intel*\n\n` +
    `📊 Instability Index: *${data.cii?.score || 'N/A'}/100* (${data.cii?.level || 'N/A'})\n` +
    `📈 Trend: *${data.cii?.trend || 'stable'}*\n` +
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Global Intel Brief - Pacifica`)}`,
