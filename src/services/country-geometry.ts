...
function normalizeGeometry(geometry: Geometry | null | undefined): [number, number][][][] {
  ...
  if (geometry.type === 'Polygon') {
    const polygon = normalizePolygonRings((geometry as { coordinates: Position[][] }).coordinates);
    return polygon.length > 0 ? [polygon] : [];
  }
  if (geometry.type === 'MultiPolygon') {
    return (geometry as { coordinates: Position[][][] }).coordinates
      .map((polygonCoords: Position[][]) => normalizePolygonRings(polygonCoords))
      .filter((polygon: [number, number][][]) => polygon.length > 0);
  }
  return [];
}
