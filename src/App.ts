import type { NewsItem, Monitor, PanelConfig, MapLayers, RelatedAsset, InternetOutage, SocialUnrestEvent, MilitaryFlight, MilitaryVessel, MilitaryFlightCluster, MilitaryVesselCluster, CyberThreat, UcdpGeoEvent } from '@/types';
import {
  FEEDS,
  INTEL_SOURCES,
  SECTORS,
...
export class App {
  ...
  private latestUcdpEvents: UcdpGeoEvent[] = [];
        ...
        if (!localStorage.getItem(FULL_PANEL_ORDER_MIGRATION_KEY)) {
          ...
          if (savedOrder) {
            try {
              ...
              localStorage.setItem(this.PANEL_ORDER_KEY, JSON.stringify([...preferred, ...rest]));
            } catch {
            }
          }
  ...
  private async checkForUpdate(): Promise<void> {
    try {
      ...
      const releaseUrl = typeof data.url === 'string' && data.url
        ? data.url
        : 'https://github.com/pacifica-fi/global-intel/releases/latest';
  ...
  private async loadIntelligenceSignals(): Promise<void> {
    ...
    try {
      const events = [
        ...
        ...this.latestUcdpEvents.slice(0, 10).map(e => ({
