...
const KEYRING_SERVICE: &str = "global-intel";
...
fn build_app_menu(handle: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let about_metadata = AboutMetadata {
        name: Some("Global Intel".into()),
        version: Some(env!("CARGO_PKG_VERSION").into()),
        copyright: Some("\u{00a9} 2026 Pacifica".into()),
        website: Some("https://app.pacifica.fi".into()),
        website_label: Some("app.pacifica.fi".into()),
        ..Default::default()
    };
    let about_item = PredefinedMenuItem::about(handle, Some("About Global Intel"), Some(about_metadata))?;
...
fn handle_menu_event(app: &AppHandle, event: tauri::menu::MenuEvent) {
    match event.id().as_ref() {
        ...
        MENU_HELP_GITHUB_ID => {
            let _ = open_in_shell("https://github.com/pacifica-fi/global-intel");
        }
        MENU_HELP_DEVTOOLS_ID => {
...
fn main() {
    tauri::Builder::default()
        ...
        .expect("error while running global-intel tauri application")
        .run(|app, event| {
            match &event {
